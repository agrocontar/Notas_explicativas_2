...
exports.listBalancoWithTotals = void 0;
const prismaClient_1 = require("../prismaClient");
const errors_1 = require("../utils/errors");
const normalizeAccountingAccount_1 = require("../utils/normalizeAccountingAccount");
const listBalancoWithTotals = async (data) => {
    try {
        // Buscar todos os itens de balanço
        const balancos = await prismaClient_1.prisma.balancoTemplate.findMany({
            orderBy: {
                group: 'asc'
            }
        });
        if (!balancos || balancos.length === 0) {
            throw new errors_1.NotFoundError('Nenhum item de balanço encontrado!');
        }
        ...
        const balancosWithTotals = await Promise.all(balancos.map(async (balanco) => {
            // Filtrar balancetes do ano atual
            const balancetesAnoAtualFiltrados = balancetesAnoAtual.filter(balancete => balanco.accountingAccounts.some((accountCode) => balancete.accountingAccount === (0, normalizeAccountingAccount_1.normalizeAccountingAccount)(accountCode)));
            // Filtrar balancetes do ano anterior
            const balancetesAnoAnteriorFiltrados = balancetesAnoAnterior.filter(balancete => balanco.accountingAccounts.some((accountCode) => balancete.accountingAccount === (0, normalizeAccountingAccount_1.normalizeAccountingAccount)(accountCode)));
            ...
            return {
                ...balanco,
                accountingAccounts: balanco.accountingAccounts.map((accountCode) => (0, normalizeAccountingAccount_1.normalizeAccountingAccount)(accountCode)),
                totalCurrentYear,
                totalPreviousYear,
            };
        }));
...
exports.listBalancoWithTotals = listBalancoWithTotals;
